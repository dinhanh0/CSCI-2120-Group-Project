...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            // Handle functionalities for each option
            switch (menuChoice) {
    ...
    private static void selectStore(Scanner scanner, List<String[]> stores, double budget) {
        // Clear the shopping cart before starting a new shopping session
        try (FileWriter fw = new FileWriter("cart.csv")) {
            fw.write(""); // Overwrite the cart file with an empty string
            System.out.println("Shopping cart has been reset.");
        } catch (IOException e) {
            System.err.println("Error resetting the shopping cart: " + e.getMessage());
            return;
        }
        ...
        // Iterate through store list and filter stores based on user input range
